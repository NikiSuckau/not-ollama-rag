...
class Converse:
    ...
    def __init__(self):
        os.environ["TOKENIZERS_PARALLELISM"] = (
            "false"  # required to run Chroma DB properly on CPU
        )
        ...
        template = (
            """You are talkative and provide lots of specific details from
            ...
            Your task is to write the next thing that """
            + self.agent_name
            + """ will say
            only. Do not write more than one message from """
            + self.agent_name
            + """. Do
            not include any prefix or quotes to the message. Answer as if you
            are """
            + self.agent_name
            + """, in the first person. If you don't know something,
        ...
        self.chain = (
            {"context": self.orchestrateRetrievers, "input": RunnablePassthrough()}
            | self.prompt
            | self.model
            | StrOutputParser()
        )
    ...
    def orchestrateRetrievers(self, query: str):
        ...
        if self.enable_doc_search:
            resultBooks = (
                self.retriever_books | self.retrieverAddBookMetadataToBookPassage
            ).invoke(query)
            result += resultBooks
    ...
    def retrieverAddDateToPageContent(self, docs):
        for d in docs:
            d.page_content = (
                self.dateToTimeAgo(d.metadata["timestamp"]) + "," + d.page_content
            )
        return docs
    ...
    def retrieverFilterByDateRelevance(self, docs):
        if len(docs) > self.RET_DATE_REL_LIST_LEN_MAX:
            ...
            updated_docs = []
            docs_tuples = map(
                lambda d: (self.dateStrToClass(d.metadata["timestamp"]), d), docs
            )
            ...
            if DEBUG_ENABLED:
                print(
                    "retrieverFilterByDateRelevance, most recent: " + str(updated_docs)
                )
    ...
    def getIsInteresting(self, query: str):
        return self.testQueryForYesNo(
            query,
            "Does this query contain some facts worth remembering, not just chit chat?",
        )
    ...
    def ask(self, query: str):
        ...
        if (
            WEB_SEARCH_ENABLED
            and len(response) <= self.DONT_KNOW_RESPONSE_LEN_LIMIT
            and re.search("don't know", response, re.IGNORECASE) != None
        ):
            if DEBUG_ENABLED:
    ...
    def getSearch(self, query: str, result_count: int = 3):
        ...
        search_results = subprocess.check_output(
            f'ddgr -n {str(result_count)} -r ie-en -C --unsafe --np "{search_query}"',
            shell=True,
        ).decode("utf-8")
    ...
    def sayIt(self, text: str):
        try:
            subprocess.check_output(
                'say "{}"'.format(text.replace('"', "").replace("'", "")), shell=True
            )
